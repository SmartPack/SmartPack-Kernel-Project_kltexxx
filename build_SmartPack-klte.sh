...
KERNEL_VERSION="stable"   # leave as such, if no specific version tag
...
mv anykernel_SmartPack/dtb release_SmartPack/

# updating kernel name

sed "s/\`echo \$LINUX_COMPILE_BY | \$UTS_TRUNCATE\`/SmartPack-Lite-Kernel-klte-[sunilpaulmathew/g" -i scripts/mkcompile_h
...
mv release_SmartPack/dtb anykernel_SmartPack/
