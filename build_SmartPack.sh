...
# 1. Properly locate Stock, UBER & Linaro toolchains (Line# 41, 43 & 45)
# 2. Select the preferred toolchain for building (Line# 47)
# 3. Set the 'KERNEL_VARIANT' (Line# 53)
...
KERNEL_VERSION="stable-v13_r5"   # leave as such, if no specific version tag
...
if [ -z "$KERNEL_VARIANT" ]; then
	...
	sed -i "s;stable;-$KERNEL_VERSION;" $BUILD_DIR/.config;
	...
	if [ -e output_kltespr/arch/arm/boot/zImage ]; then
		echo -e $COLOR_GREEN"\n copying zImage to anykernel directory\n"$COLOR_NEUTRAL
		cp output_kltespr/arch/arm/boot/zImage $ANYKERNEL_DIR/
		# compile dtb if required
		if [ "y" == "$COMPILE_DTB" ]; then
			echo -e $COLOR_GREEN"\n compiling device tree blob (dtb)\n"$COLOR_NEUTRAL
		...
		echo -e $COLOR_RED"\n Building for kltespr failed. Please fix the issues and try again...\n"$COLOR_NEUTRAL
	fi
	# klteduos
